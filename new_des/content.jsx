...
const POST_BODY = [
  ...
  { type: "tip", title: "Hey, I found this out", text: "Pin doesn't actually do anything at runtime. It's purely a compile-time promise — the type system enforces that you can't get an &mut T out of a Pin<&mut T> without unsafe (unless T: Unpin)." },
  { type: "rabbithole", to: "lifetimes-vs-borrow", title: "Lifetimes are not the borrow checker.", blurb: "Pin is one of those types that makes lifetime errors look like borrow errors. They are not the same thing — and the distinction matters once you start writing self-referential futures by hand." },
  { type: "warn", title: "I think this is a mistake", text: "Earlier I said Box::pin guarantees stability. That's only true because Box owns a heap allocation that doesn't move. Pin<Box<T>> is stable; Pin<&mut T> on a stack value is only stable until the function returns. Don't conflate them." },
  { type: "ul", items: [
